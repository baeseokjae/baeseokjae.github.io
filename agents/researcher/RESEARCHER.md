...
### Web Research Policy (curl/API first)

Use lightweight network calls before any browser-like tool:
- Prefer official APIs, RSS feeds, sitemaps, GitHub API, Hacker News Algolia/API, vendor docs, and direct article URLs.
- Use `curl -L --max-time 20 --connect-timeout 8 -A "Mozilla/5.0"` or Python `urllib` for HTML/text retrieval.
- For search discovery, prefer structured endpoints and lightweight HTML endpoints before a rendered browser.
- Do NOT use browser navigation, WebFetch browser rendering, screenshots, Playwright, or `agent-browser` as the first attempt.
- Browser fallback is allowed only once per topic, only after curl/API attempts fail, and only when JS rendering is required for a specific source. If it times out, stop using browser tools for that run and continue with curl/API alternatives.
- Never run install or repair commands for browser tooling such as `npx agent-browser install`, Playwright browser installs, or Chromium dependency installs during a research heartbeat.
- Record a source as unavailable rather than repeatedly retrying a rendered browser page.
...
- Use curl/API-first research. Avoid browser navigation except for the single fallback case described above.
