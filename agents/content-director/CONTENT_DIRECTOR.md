...
4. **Queue check**: If queued topics < 1 (LOW_WATERMARK; the queue is intentionally throttled to one visible topic per dispatch):
...
5. If queued topics >= 1: Pick up to 8 topics with status "queued" (highest priority first)
...
2. **Queue check**: If queued topics < 1:
   - Wake the Strategist (same curl as above)
   - Continue with dispatch (don't block)
3. **Smart busy check** — block if any Research, Write, or Publish is active (todo OR in_progress):
   - Query `GET /api/companies/{companyId}/issues?status=in_progress` → check for Research:, Write:, or Publish: titles
   - Query `GET /api/companies/{companyId}/issues?status=todo` → check for Research:, Write:, or Publish: titles
   - **EXCLUDE** issues assigned to disabled agents (SEO/Thumbnail/Supervisor). Those agents are offline and their issues should be cancelled, not counted as "active".
     ...
     - Supervisor agent ID: bf9adfce-4e5f-4c4e-bb6c-8a35086e15b1
   - If ANY valid Research, Write, or Publish sub-issue is in_progress **or** todo → "Research/Write/Publish active, skipping." and exit
...
- Auth: local-board only (`X-Paperclip-Local-Board: true`). Do **not** send `Authorization: Bearer ...` from ContentDirector; bearer auth triggers Paperclip's cross-agent "agent can only invoke itself" wakeup restriction.
