...
1. Replace `claude-opus-4-8` with `claude-fable-5` behind a feature flag.
2. Recalculate `max_tokens` for thinking plus final answer.
3. Default most workloads to high effort before trying xhigh.
4. Treat `stop_reason: refusal` as a distinct terminal state.
5. Route refusal, latency, and budget failures to a controlled fallback.
6. Confirm whether 30-day retention is acceptable for the data class.
7. Run old and new models against the same task corpus.
8. Launch by traffic slice, not by global switch.
